# Clasificación usando maquina de soporte vectorial
...
# Ajuste de las SVM
clasificador = SVC(kernel = "linear", random_state = 1234)
modelo1 = clasificador.fit(x_train,y_train)

#predicción
y_pred = modelo1.predict(x_test)
#print(y_pred)

#Evualación usando matriz de confusión
...
for i, j in enumerate(np.unique(y_set)):
    plt.scatter(X_set[y_set == j, 0], X_set[y_set == j, 1], c = ListedColormap(('red', 'darkblue'))(i), label = j)
plt.title('SVM (Datos Entrenamiento)')
plt.xlabel('Edad')
plt.ylabel('Salario')
plt.legend()
plt.show()

X_set, y_set = escalador.inverse_transform(x_test), y_test
