...
varianza_expl = lda.explained_variance_ratio_
...
plt.contourf(X1, X2, clasificador.predict(np.array([X1.ravel(), X2.ravel()]).T).reshape(X1.shape),
             alpha = 0.75, cmap = ListedColormap(('c', 'm',"y")))
plt.xlim(X1.min(), X1.max())
plt.ylim(X2.min(), X2.max())
for i, j in enumerate(np.unique(y_set)):
    plt.scatter(X_set[y_set == j, 0], X_set[y_set == j, 1], c = ListedColormap(('c', 'm',"y"))(i), label = j)
plt.title('Regresion Logistica(Datos Entrenamiento usando LDA) ')
plt.xlabel('LD1')
plt.ylabel('LD2')
plt.legend()
plt.show()
